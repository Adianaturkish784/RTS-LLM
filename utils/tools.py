...
def adjust_learning_rate(accelerator, optimizer, scheduler, epoch, args, printout=True):
    if args.lradj == 'half_decay':
        lr_adjust = {epoch: args.learning_rate * (0.5 ** ((epoch - 1) // 1))}
    elif args.lradj == 'one_cycle':
        lr_adjust = {epoch: scheduler.get_last_lr()[0]}
    else:
        raise ValueError(f"Unsupported learning rate adjustment strategy: {args.lradj}")
...
def vali(args, accelerator, model, vali_data, vali_loader, criterion, mae_metric, max_batches=None):
    total_loss = []
    total_mae_loss = []
    model.eval()
    with torch.no_grad():
        for i, batch in tqdm(enumerate(vali_loader)):
            if max_batches is not None and i >= max_batches:
                break
            if len(batch) == 8:
                batch_x, batch_y, batch_x_mark, batch_y_mark, _, _, _, _ = batch
            else:
